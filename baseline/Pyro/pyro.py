...
def ascend(L, e_max, P, Attr_list):
    #print(Attr_list)
    X = [L]
    e_X = error(AFD([], X), P)
    while True:
...
def Pyro(R, e_phi, e_v):
    ...
    for i in range(-1, len(R[0])):
        ...
        for j in range(len(R[0])):
            ...
            x = ascend(j, error, pli_cache, Attr_List)
...
if __name__ == "__main__":
    ...
    print("Cnt:", len(ans))
    pli_cache = PLI(Data2)
    print("Score:", get_Score(pli_cache, ans))
    # pli.print()
    '''for i in ans:
        print(i.LHS,i.RHS)'''
    ...
    start = time.time()
    Data1=Data[:int(len(Data)*4/5)]
    Data2=Data[int(len(Data)*0.8):]
    ans, pli = Pyro(Data, 0, 0)
    end = time.time()
    print("No.1")
    print("Glass:", Data.shape)
    print("Time:", (end - start))
